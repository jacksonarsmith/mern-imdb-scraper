    ...
    async scrapeAndPopulateDatabase() {
      try {
        ...
        $('li.ipc-metadata-list-summary-item').each(function (index, element) {
            const $movieElement = $(element).find('.sc-53c98e73-4.gOfInm.dli-parent');
            ...
            const rating = $movieElement.find('[data-testid="ratingGroup--imdb-rating"]').text().trim().match(/\d+\.\d+/)[0];
            ...
            const votes = $movieElement.find('.sc-53c98e73-0.kRnqtn').text().trim().replace('Votes', '').trim();
